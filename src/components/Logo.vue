<template>
  <svg viewBox="0 0 140 32" fill="none" aria-hidden="true" v-bind="$attrs">
    <title>Commit</title>
    <defs>
      <linearGradient
        :id="maskGradientId"
...
const id = useId()
const maskId = computed(() => `${id}-m`)
const maskGradientId = computed(() => `${id}-g`)
const gradientUrl = computed(() => `url(#${id}-g)`)
const maskUrl = computed(() => `url(#${id}-m)`)
</script>
